...
*/
void toytree_TLorentz()   //we always name the main function the name same as file
{  
	...
	while (reader.Next())
	{
		...
		{
            muon1.SetPtEtaPhiM(pt1[i],eta1[i],phi1[i],Mmuon);
            muon2.SetPtEtaPhiM(pt2[i],eta2[i],phi2[i],Mmuon);
            Mass=(muon1+muon2).M();
...
*/
	...
	h_mass -> Draw();
    TFile *output= new TFile("Results_TLorentz.root","recreate");
    h_mass -> Write();
    //Draw the legend
